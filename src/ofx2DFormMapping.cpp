...
void ofx2DFormMapping::setNewPosition(ShapeProperty *sp,int index){

	if(shapes.size()>0){
		ofPtr<ofx2DMappingObject> obj = parent_projector->getMappingObject(index);


		if(ofPtr<ofx2DMappingShape> shape = std::dynamic_pointer_cast<ofx2DMappingShape>(obj)) {
			for(int i=0;i<4;i++){
				float fposX = sp->getCornerX(i);
				float fposY = sp->getCornerY(i);
				shape->dst[i].x = fposX;
				shape->dst[i].y = fposY;
			}
		}
